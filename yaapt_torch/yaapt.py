# -*- coding: utf-8 -*-
"""
This is a ported version of the YAAPT algorithm for PyTorch.
The original NumPy/SciPy/Python program was written by Bernardo Jose B.
(https://github.com/bjbschmitt).
...
While this version is slower than the NumPy/Python implementation when
estimating the F0 of a single signal (due to single-core processing), it has
the significant advantage of not hogging all CPU threads for a single signal
computation. Therefore, it is possible to use this version in a Torch parallel
dataloader, which was impractical with the NumPy/SciPy implementation. This
version is also batch compatible and torch.jit-able.
...
For pytorch, it must be noticed that this version is almost equal to the
original, but some changes were made to make it only rely on pytorch wihtout
numpy/scipy. The results are colose the the numpy/scipy, but slightly differ.
"""
...
if __name__ == "__main__":
    ...
    audio, sr = torchaudio.load("/data/pchampio/Librispeech/LibriSpeech/train-clean-360/1028/133393/1028-133393-0001.flac")
